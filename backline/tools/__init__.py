...
from backline.tools.scan import build_scan_anomalies_tool
...
def build_all_tools(ctx: ToolContext) -> list[Tool[Any]]:
    """Every registered tool, one binding each (§4.3 plus the Phase 4 Reconciler
    additions); agents get per-agent subsets via ``backline.agents.configs``."""
    return [
        ...
        build_scan_anomalies_tool(ctx),
        build_compute_allocations_tool(ctx),
